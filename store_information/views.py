...
def upload_item(request):
    if request.method != 'POST':
        ...
        if form.is_valid():
            ...
            new_item.save()
            return redirect('store_information:gallery')
    context = {'form': form}
...
@login_required()
def estore(request):
    contents = ItemEstore.objects.filter(owner=request.user)
    context = {'contents': contents}
    return render(request, 'store_information/estore.html', context)
...
def gallerie(request, gallerie_id):
    ...
    items2 = Item.objects.filter(location='Kwekwe')
    user = request.user
    wishlist = user.my_wishlist.all()
    for wish in wishlist:
        wishes.append(wish.product)
    context = {'gallerie': gallerie, 'items2': items2, 'user': user, 'wishes': wishes}
...
def product_estore(request, product_estore_id):
    ...
    context = {'product_estore': product_estore}
